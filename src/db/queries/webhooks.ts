...
export async function createWebhookDB(
  userId: string,
...
export async function getWebhookByIdDB(
  webhookId: string,
...
export async function getWebhookBySourceDB(
  source: string,
): Promise<Webhook | undefined> {
  const webhook = await db.query.webhooks.findFirst({
...
export async function getAllWebhooksByUserIdDB(
  userId: string,
): Promise<Webhook[]> {
  const webhooks = await db.query.webhooks.findMany({
...
export async function updateWebhookDB(
  webhookId: string,
