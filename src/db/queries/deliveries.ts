...
export async function increaseAttemptsNumberDB(id: string): Promise<void> {
  await db
    .update(deliveries)
    .set({ attemptsNumber: sql`${deliveries.attemptsNumber} + 1` })
    .where(eq(deliveries.id, id));
}

export async function updateDeliveryStatusDB(
  id: string,
  ...
  if (status === "SUCCESS")
    await db
      ...
      .where(eq(deliveries.id, id));
  else await db.update(deliveries).set({ status }).where(eq(deliveries.id, id));
}
